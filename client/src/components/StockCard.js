...
const StockCard = ({ stock, onClick }) => {
  ...
  return (
    ...
    >
      ...
      <p>
        <strong>Average Purchase Price:</strong> $
        {averagePurchasePrice ? averagePurchasePrice.toLocaleString("en-US", { minimumFractionDigits: 2 }) : "0.00"}
      </p>
      <p>
        <strong>Total Cost:</strong> $
        {totalCost ? totalCost.toLocaleString("en-US", { minimumFractionDigits: 2 }) : "0.00"}
      </p>

      {/* Display individual profit/loss */}
      <p className={profitLoss >= 0 ? "text-green-600" : "text-red-600"}>
        <strong>Profit/Loss:</strong> ${profitLoss.toLocaleString("en-US", { minimumFractionDigits: 2 })} 
